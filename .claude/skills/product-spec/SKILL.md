...
6. 문제, 사용자 니즈, 솔루션에는 파일명, 코드 식별자, API 경로를 쓰지 않는다. 읽는 사람은 디자이너와 사업 담당자도 포함. 식별자는 용어 표와 구현 상세에만
...
`references/review-checklist.md`로 초안을 판정한다. 결과 표는 출력하지 않고, 질문 전에 "확인 필요 {⚠️와 빈칸 수}건 중 착수 전 {A}건을 먼저 묻습니다" 한 줄만. 계수 단위와 A 판정은 `references/question-map.md`의 규칙
...
4. 연쇄 갱신: 페이지 추가나 삭제 → 변경 페이지 표와 메뉴 구조 표. 흐름 변경 → `.mmd` 수정과 두 그림 재렌더. 범위 확장 → 구현 범위 요약 첫 불릿과 솔루션과 현재 vs 변경 표. 모든 수정 뒤 첫 줄의 착수 전과 전체 건수를 다시 센다. `prototype.html`이 있으면 바뀐 행의 설명 패널 문구를 같이 고치고, 못 고치면 프로토타입 상단에 `⚠️ 스펙 {날짜} 기준, 최신 아님` 배너를 넣는다
...
- 화면마다 설명 패널: 화면 한쪽에 접고 펼 수 있는 패널을 두고, 그 화면에 해당하는 구현 상세 행(진입점, 기능, 요구사항 번호 불릿, ⚠️)을 스펙 문구 그대로 보여준다. 행과 화면은 화면 열에서 `(신규 생성)` 같은 접미사를 뗀 이름으로 맞춘다. 화면에 대응하지 않는 행(서버 스케줄, 배치, 발송, 집계)은 '화면 밖 동작' 패널 하나에 모아 둔다. 디자이너와 개발자에게 프로토타입만으로 설명할 수 있게 하기 위함. 요구사항 항목을 누르면 해당 UI 요소가 강조되면 더 좋다. 파일에 사내 API 경로가 들어가므로 스펙 폴더 밖으로 보낼 때는 공유 범위를 확인
