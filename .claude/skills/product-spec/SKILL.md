...
- 흐름도는 `references/diagram.md`대로 가로형으로 `diagrams/{이름}.mmd`를 쓰고 첨부용과 블록용 두 그림을 모두 렌더. 본문에는 `![사용자 흐름](diagrams/{이름}.png)`
...
- 화면마다 설명 패널: 화면 한쪽에 접고 펼 수 있는 패널을 두고, 그 화면에 해당하는 구현 상세 행(진입점, 기능, 요구사항 번호 불릿, ⚠️)을 스펙 문구 그대로 보여준다. 디자이너와 개발자에게 프로토타입만으로 설명할 수 있게 하기 위함. 요구사항 항목을 누르면 해당 UI 요소가 강조되면 더 좋다
