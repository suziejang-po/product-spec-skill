...
3. 코드 식별자(이벤트명, API 경로, 항목 이름, 컴포넌트)는 출처에서 그대로 복사. 유추한 이름 금지. 도구로 확인하면 (기존) 또는 (신규 제안), 도구가 없으면 (문서명), 문서에는 있는데 도구에서 못 찾으면 (문서명, 미확인)과 `⚠️`
...
5. 분량. 구현 상세의 요구사항 셀은 번호 불릿으로 개수 상한 없음(8개를 넘으면 기능을 나눔). 그 외 모든 셀은 3줄 이내. 문제와 솔루션은 한 줄 불릿, 사용자 니즈는 이해관계자별 한 줄. 문단 서술 없음. 결정 이력과 검토 서사는 본문에 넣지 않음. 구현 범위 요약은 표 4개까지(비주얼, 현재 vs 변경, 변경 페이지, 메뉴 구조), 나머지 섹션은 표 1개(구현 상세는 이벤트 표와 API 표 추가 허용)
6. 문제, 사용자 니즈, 솔루션에는 파일명, 코드 식별자, API 경로를 쓰지 않는다. 읽는 사람은 디자이너와 사업 담당자도 포함. 식별자는 용어 표, KR 표의 측정과 근거, 구현 상세에만
7. 문체는 `references/writing-style.md`. 개조식 명사 종결, 금지 기호와 금지어와 은어 없음. 저장 뒤 lint 통과
8. 다이어그램은 그림. 가로형(flowchart LR) 기본. 코드 블록을 본문에 넣지 않음
9. 토큰은 실행하는 사람 본인의 환경변수만 읽음. 파일이나 문서에 적지 않음
...
- 비대화 환경(서브에이전트, 배치)이면 "채팅에 출력"은 스펙 폴더의 `draft.md`, `questions.md` 저장으로 대신하고, 답이 올 수 없으므로 도메인 팩 질문과 Phase 3 질문은 저장만 한 뒤 답 없는 항목을 전부 ⚠️로 두고 Phase 4까지 마치고 끝낸다
...
- 흐름도는 분기가 2개 이상일 때만. `references/diagram.md`대로 가로형으로 `diagrams/{이름}.mmd`를 쓰고 첨부용과 블록용 두 그림을 모두 렌더. 본문에는 `![사용자 흐름](diagrams/{이름}.png)`
- 기존 제품을 바꾸는 스펙이면 구현 범위 요약에 현재 vs 변경 비교표. 출처는 기존 스펙이나 코드
...
`references/review-checklist.md`로 초안을 판정한다. 결과 표는 출력하지 않고, 질문 전에 "확인 필요 {⚠️ 수}건 중 착수 전 {A}건을 먼저 묻습니다" 한 줄만. 계수 단위와 A 판정은 `references/question-map.md`의 규칙
...
3. 자가 검증 (저장 뒤)
   ...
   - 수치마다 근거 또는 ⚠️가 있는가. 같은 수치가 서로 다른 값으로 적히지 않았는가
   - A 등급 확인 사항은 `⚠️ 착수 조건:` 꼴이고, 첫 줄의 두 건수가 본문의 `⚠️ 착수 조건` 수와 ⚠️ 수와 같은가 (lint가 대조)
   - 프로토타입이 있으면 구현 상세의 모든 행이 화면 패널 또는 화면 밖 동작 패널에 한 번씩 들어갔는가
   - 문제, 사용자 니즈, 솔루션에 파일 경로, 슬래시로 시작하는 경로, 밑줄 두 개가 든 이벤트명, 테이블명이 없는가
   - lint 오탐(실제 UI 카피, 컴포넌트 이름)은 지우지 말고 `<!-- lint-skip: 사유 -->`로 남긴다
   - 흐름도가 그림 파일로 들어갔고 두 그림이 모두 있는가
4. 문서 첫 줄의 "착수 전 확인 필요 {A}건, 확인 필요 전체 {⚠️}건"을 실제 개수로 채우고, 안내: "저장: docs/specs/{슬러그}/spec.md. 착수 전 확인 필요 {A}건, 확인 필요 전체 {⚠️}건". A 등급이 남아 있으면 항목 이름을 한 줄씩 덧붙임
...
4. 연쇄 갱신: 페이지 추가나 삭제 → 변경 페이지 표와 메뉴 구조 표. 흐름 변경 → `.mmd` 수정과 두 그림 재렌더. 범위 확장 → 구현 범위 요약 첫 불릿과 솔루션과 현재 vs 변경 표. 모든 수정 뒤 첫 줄의 착수 전과 전체 건수를 다시 센다. `prototype.html`이 있으면 바뀐 행의 설명 패널 문구를 같이 고친다. 행이 삭제돼 대응 패널이 없거나 화면이 새로 생겨 프로토타입에 없는 경우에만 프로토타입 상단에 `⚠️ 스펙 {날짜} 기준, 최신 아님` 배너를 넣는다
5. 새로 생긴 빈칸은 Phase 2~3을 그 부분에만 적용
6. 안내: "갱신: docs/specs/{슬러그}/spec.md (변경 섹션 N개, 착수 전 확인 필요 M건)". Confluence Page ID가 있으면 `references/confluence.md`의 갱신 절차

## 아티팩트 생성

단일 HTML 파일(Vanilla JS와 CSS, 외부 CDN 없음, 오프라인 동작). 스펙과 같은 응답에 넣지 않고 별도로 만듦
- 화면 프로토타입: `<!DOCTYPE html>` 완전한 문서. 시스템 폰트 'Pretendard', -apple-system, sans-serif. CSS 변수로 Wanted Montage 색(`--color-primary: #3366FF` 등). 화면 전환은 class 토글. 입력 항목은 검증과 오류 문구 동작. 플랫폼별 차이가 있으면 뷰포트 전환 버튼
- 화면마다 설명 패널: 화면 한쪽에 접고 펼 수 있는 패널을 두고, 그 화면에 해당하는 구현 상세 행(진입점, 기능, 요구사항 번호 불릿, ⚠️)을 스펙 문구 그대로 보여준다. 행과 화면은 화면 열에서 `(신규 생성)` 같은 접미사를 뗀 이름으로 맞춘다. 화면에 대응하지 않는 행(진입점이 서버 스케줄, 배치, 발송, 집계인 행. 화면 열은 `해당 없음 ({종류})`로 적음)은 '화면 밖 동작' 패널 하나에 모아 둔다. 디자이너와 개발자에게 프로토타입만으로 설명할 수 있게 하기 위함. 요구사항 항목을 누르면 해당 UI 요소가 강조되면 더 좋다. 파일에 사내 API 경로가 들어가므로 스펙 폴더 밖으로 보낼 때는 공유 범위를 확인
