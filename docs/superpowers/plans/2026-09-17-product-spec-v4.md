...
## 실행 기록 (2026-09-21, v4.1 ~ v4.1.2)

수지님 피드백(포털 스펙 리뷰)에서 옮긴 규칙
- 문제는 한 줄 불릿 3~4개(현상, 영향, 계기), 파일명과 코드 식별자 금지. 사용자 니즈는 이해관계자별 한 줄. 솔루션은 개조식 불릿
- 기존 제품을 바꾸면 현재 vs 변경 비교표. 흐름도 가로형. 프로토타입 화면별 설명 패널
- 질문 탈출구("그냥 만들어") 제거, 문서 첫 줄에 착수 전 확인 필요 건수
- Cursor 세션이 넣은 은어 검사와 행 번호 참조 검사를 레포에 합침

적대적 리뷰 2회
- 1회차 높음 5건: 예시 파일이 새 규칙 위반, 가로형이 블록에서 판독 불가, 행 번호 검사 오탐, 착수 전 건수 정의 불일치, 수정 모드 첫 줄 미갱신. 전부 반영
- 2회차 높음 2건: 1회차 반영이 블록 경로를 통째로 끈 회귀(첨부용 그림 폭 조건) → 블록용 그림 기준으로 수정하고 PNG 양자화 내장. A 등급 판정 불가 → `⚠️ 착수 조건:` 표기 강제와 lint 건수 대조. 중간 9건과 낮음 9건도 반영(lint-skip 종류 지정, 체크리스트 대항목, README와 안내 정합, timeout 등)
- 검증: 가로 흐름도는 안내 상자, 세로 상태도는 블록으로 들어가는 것을 변환기로 확인. lint 오탐 probe 통과

남은 것
- 첨부 경로(토큰)는 여전히 실사용 미검증
- 프로토타입 설명 패널 규칙은 새 스펙에서 첫 실행 때 확인 필요
